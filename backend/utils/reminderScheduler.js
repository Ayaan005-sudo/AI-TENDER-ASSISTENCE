...
async function checkAndSendReminders() {
  try {
    ...
    for (const tender of tenders) {
      if (tender.deadline && new Date(tender.deadline) < today) continue;
      if (!Array.isArray(tender.reverseTimeline)) continue;
      ...
      for (const item of tender.reverseTimeline) {
        ...
        if ([7, 3, 1].includes(diff)) {
          ...
          console.log(
            `Reminder sent → ${tender.tenderName} | ${item.task} | in ${diff} days`
          );
...
/**
 * Start cron (FIXED FOR RENDER)
 */
function startReminderScheduler() {
  // Run at 9 AM IST (3:30 AM UTC)
  ...
  console.log('Reminder scheduler started (IST 9 AM)');
}
